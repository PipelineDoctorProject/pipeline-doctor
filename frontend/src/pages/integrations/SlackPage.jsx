...
export default function SlackPage() {
  ...
  async function handleDisconnect() {
    if (!window.confirm("Disconnect Slack for this workspace? Incident alerts will fall back to email until Slack is reconnected.")) {
      return;
    }
        ...
        <div className="flex flex-col gap-4 border-b border-slate-200 px-6 py-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {status?.can_manage ? (
              ...
              >
                <Link2 size={15} />
                {status?.connected ? "Reconnect Slack" : connecting ? "Opening Slack..." : "Connect Slack"}
              </button>
            ) : null}
          </div>
        </div>
        ...
        <>
          <section className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
            <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-[0_12px_34px_rgba(15,23,42,0.04)]">
              ...
              {!status?.connected ? (
                <div className="space-y-4 rounded-md border border-dashed border-slate-200 bg-slate-50 p-5">
                  <div>
                ...
                <div className="space-y-4">
                  <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4">
                    <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-emerald-700">
                      {status.default_channel ? "Slack connected" : "Slack app authorized"}
                    </p>
                    <p className="mt-2 text-[14px] leading-6 text-emerald-800">
                      {status.default_channel
                        ? "Incident alerts are ready for the selected channel."
                        : "Choose a default channel to complete the Slack connection."}
                    </p>
                  </div>
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-3">
                        <button
                          ...
                          className="inline-flex h-10 items-center gap-2 rounded-md bg-slate-950 px-4 text-[13px] font-semibold text-white transition hover:bg-slate-800 disabled:opacity-50"
                        >
                          {status.default_channel ? "Update Channel" : "Connect Channel"}
                        </button>
                        <button
                          onClick={handleDisconnect}
                          className="inline-flex h-10 items-center gap-2 rounded-md border border-rose-200 bg-rose-50 px-4 text-[13px] font-semibold text-rose-700 transition hover:bg-rose-100"
