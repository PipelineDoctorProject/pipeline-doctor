...
def list_incidents(
    ...
    if drift_ids:
        drift_by_id = {
            finding.id: finding
            for finding in db.query(DriftFinding).filter(DriftFinding.id.in_(drift_ids)).all()
        }

    return [
        _serialize_incident(incident, drift_by_id.get(incident.finding_id))
        for incident in incidents
    ]


def _serialize_incident(incident: Incident, drift_finding: DriftFinding | None = None):
    rca_report = _parse_rca_report(incident.description)
    return {
        "id": incident.id,
        "run_id": incident.run_id,
        "title": incident.title,
        "description": _human_description(incident, rca_report),
        ...
        "guidance": _build_guidance(incident, drift_finding, rca_report),
...
def _human_description(incident: Incident, rca_report: dict | None):
    if not rca_report:
...
def _build_guidance(
    ...
    if rca_report:
        ...
        return {
            "source": "llm" if provider != "fallback" else "deterministic",
            "model": model,
            "cause": rca_report.get("summary") or "The RCA engine grouped the run's failed monitoring signals.",
            "action": rca_report.get("recommendation") or "Review the evidence and fix the highest severity issue first.",
        }
    ...
    if drift_finding:
        ...
        statistical_note = (
            ...
            else "KS p-value is not significant, so confirm with business context before treating it as bad data."
        )
        return {
            "source": "metric_interpreter",
            "model": None,
            "cause": (
                f"{feature} has {strength} drift: PSI={psi:.4f}, KS={ks:.4f}, "
                f"score={score:.4f}. {statistical_note}"
            ),
            "action": (
                f"Compare {feature} in the current batch against the baseline window. "
                "Refresh the baseline only when the new population is expected; otherwise inspect the upstream feed."
            ),
        }
    ...
    return {
        "source": "metric_interpreter",
        "model": None,
        "cause": "One or more monitoring signals crossed the incident threshold.",
        "action": "Review Data Quality and Drift details together before retraining or approving a new baseline.",
    }
